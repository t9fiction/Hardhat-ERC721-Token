...
modifier _saleStatus() {
    require(saleStart,"Sale is Off currently.");
    _;
}
    // ___________________ Starting the sale ________________________
  function startSale(uint _saleDays) public onlyOwner{
      require(_saleDays <= 30 days,"The sale has to be less or equal to 30 days");
      saleStart = true;
  }  
  // ___________________ Stoping the sale ________________________
  function stopSale() public _saleStatus onlyOwner{
      saleStart = false;
  }  
  ...
  function buyAXYToken(uint _quantity) payable external _saleStatus {
        ...
        for(uint i = 1; i<= _quantity; i++){
            ...
            mintNdIncrementSupply(msg.sender, currentPublicTokenID);
        }
  }
